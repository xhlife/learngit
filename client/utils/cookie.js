...
function setCookieFromHeader(setCookieStr) {
    ...
    setCookieArr = setCookieStr.replace(/(Expires=[A-Za-z]{3}),/ig, '$1_').split(',').map(item => item.replace(/(Expires=[A-Za-z]{3})_/ig, '$1,'))
...
function parseCookieItem(cookieStr) {
    ...
    let firstParamArr = params.shift().split('=')

    // 这里兼容下奇葩cookie值，实际应该 ===2 比较合适
    if (firstParamArr.length < 2) return console.error('cookie value error ', cookieStr)

    cookieItem.name = firstParamArr[0]
    cookieItem.value = firstParamArr[1]

    params.forEach(item => {
        if (item.match(/^\s*Expires=/i)) cookieItem.expires = (new Date(item.split('=')[1])).toGMTString()
